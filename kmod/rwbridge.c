...
/* ── TEMPORARY resolver + JK ops (revert after experiment) ──────────────
...
int rw_set(const char *val, const struct kernel_param *kp)
{
    ...
    case 'J': {
        /* TEMPORARY (revert): resolve + selfcheck + resolve
         * find_task_by_vpid. -100 scan fail, -101 locate/selfcheck fail,
         * -102 unresolvable, 1 resolved (addr in out). */
        xulong fa;
        if (!xsetup()) {
        ...
        fa = xresolve(&xg_ctx, "find_task_by_vpid");
        if (!fa) {
            rw_status = -102;
            rw_text_len = 0;
            rb_spin_unlock();
    ...
    case 'K': {
        /* TEMPORARY (revert): call find_task_by_vpid(1), null-check only
         * (no deref — no offsets needed). 0 + out=ptr on non-null,
         * -EIO on null, -110 without ctx. Exact kernel prototype for the
         * internal KCFI check. */
        struct task_struct *tp;
        typedef struct task_struct *(*xfind_t)(int);
        xfind_t fn;
        xulong fa;
        if (!xg_ok && !xsetup()) {
            rw_status = -110;
