 ...
 *
 * Layout constants are supplied per target kernel via kmod_offsets.h
 * (generated by CI from that kernel's kheaders/asm-offsets), since
 * task_struct/mm_struct field offsets and VA_BITS are not part of the GKI
 * stable-KMI and vary per kernel major. The arch constants below assume
 * arm64 4K pages, CONFIG_PGTABLE_LEVELS=3 (39-bit VA) — common to Android
 * 5.10..6.x. One .ko is built per kernel major and bundled; run.sh insmods
 * the one whose vermagic matches the target device. Defaults in
 * kmod_offsets.h are verified for the BlueStacks emulator:
 *   5.15.137-v5.21.770-optimizations-5.21.771.4051
 *     task_struct.tasks=1232 task_struct.(pid-field)=1496 task_struct.mm=1312
...
/* Per-kernel layout constants (struct offsets + VA_BITS-derived PAGE_OFFSET
 * and USER_VA_TOP) come from kmod_offsets.h, generated per target kernel by
 * CI (.github/workflows/build.yml). These are NOT part of the GKI stable-KMI
 * and differ per kernel major, so they cannot be hardcoded once for all
 * majors. The arch constants below (4K pages, 3-level 39-bit walk) ARE
 * common to arm64 Android 5.10..6.x. */
#include "kmod_offsets.h"
